...
4. Pseudocode 1, due to its straightforward looping condition of checking if the next item in the list is bigger than the previous item.
5. The algorithm must check if the user inputs an empty list, or a list with value types that isn’t integer or float. The algorithm must also have a way to avoid errors if those inputs are detected.
6. Pseudocode 1, since it is easy to understand its process and you could modify it to add new features without any issues.
